...
abstract class NCFunderOpenHandler(info: RemoteNodeInfo, fakeFunding: MakeFundingTxResponse, fundingFeeratePerKw: FeeratePerKw, cw: LNParams.WalletExt, cm: ChannelMaster) {
  ...
  private val makeChanListener = new ConnectionListener with ChannelListener { me =>
    ...
    override def onOperational(worker: CommsTower.Worker, theirInit: Init): Unit = {
      ...
      val cmd = INPUT_INIT_FUNDER(info, tempChannelId, fakeFunding, 0L.msat, fundingFeeratePerKw, initialFeeratePerKw, localParams, theirInit, 0.toByte, stickyChannelFeatures)
