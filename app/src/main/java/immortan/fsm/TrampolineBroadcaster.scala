...
//      val otherChannels = usableChannels.filterNot(_.commits.remoteInfo.nodeId == info.nodeId)
//      val canSendOut = otherChannels.map(_.commits.availableForSend * maxRoutableRatio).sum
//      val status = templateTrampolineOn.copy(maxMsat = canSendOut)
//
...
//    scala.concurrent.Future(broadcasters -= worker.info.nodeId)
  ...
  def doProcess(msg: Any): Unit = ???

//  def doProcess(msg: Any): Unit = (msg, state, data) match {
//    case (CMDBroadcast, ROUTING_ENABLED, routingOn: RoutingOn) =>
//      // First make a map with updated values, then broadcast differences, then update map to a new one
