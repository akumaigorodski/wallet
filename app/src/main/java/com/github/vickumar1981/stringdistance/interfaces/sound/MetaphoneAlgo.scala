...
private[stringdistance] object MetaphoneAlgo {
  ...
  private val deduplicate: (Array[Char] => Array[Char]) = (ca) =>
    if (ca.length <= 1) ca
      ...
      ca.sliding(2).withFilter(a => a(0) == 'c' || a(0) != a(1)).map(_(0)).toArray[Char] :+ ca.last

  @annotation.tailrec
  private val transcode: ((Array[Char], Char, Array[Char], Array[Char]) => Array[Char]) =
    (l, c, r, o) =>
      if (c == '\u0000' && r.length == 0) o
      else {
        ...
        val t = {
          (c: @annotation.switch) match {
            ...
            case 'b' =>
              if (l.length >= 1 && l.last == 'm' && r.length == 0) shift(1, o)
              else shift(1, o :+ 'b')
            case 'c' =>
            ...
            case 'h' =>
              if ((l.length >= 1 && lowercaseVowel.contains(
                  l.last) && (r.length == 0 || !lowercaseVowel.contains(r.head)))
                || (l.length >= 2 && l.last == 'h'
            ...
            case 'p' =>
              if (r.length >= 1 && r.head == 'h') shift(2, o :+ 'f') else shift(1, o :+ 'p')
            case 'q' => shift(1, o :+ 'k')
            case 's' =>
            ...
            case 'v' => shift(1, o :+ 'f')
            case 'w' | 'y' =>
              if (r.length == 0 || !lowercaseVowel.contains(r.head)) shift(1, o)
              else shift(1, o :+ c)
