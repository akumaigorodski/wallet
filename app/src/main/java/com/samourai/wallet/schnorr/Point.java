...
public class Point  {
    ...
    public static Point add(Point P1, Point P2) {
        ...
        BigInteger lam = null;
        if(P1.equals(P2)) {
            BigInteger base = P2.getY().multiply(BigInteger.ONE.add(BigInteger.ONE));
            lam = (BigInteger.valueOf(3L).multiply(P1.getX()).multiply(P1.getX()).multiply(base.modPow(p.subtract(BigInteger.ONE.add(BigInteger.ONE)), p))).mod(p);
        }
        else {
            BigInteger base = P2.getX().subtract(P1.getX());
            lam = ((P2.getY().subtract(P1.getY())).multiply(base.modPow(p.subtract(BigInteger.ONE.add(BigInteger.ONE)), p))).mod(p);
        }
    ...
    public static boolean hasEvenY(Point P) {
        return P.getY().mod(BigInteger.ONE.add(BigInteger.ONE)).compareTo(BigInteger.ZERO) == 0;
    }

    public static boolean isSquare(BigInteger x) {
        return x.modPow(p.subtract(BigInteger.ONE).mod(BigInteger.ONE.add(BigInteger.ONE)), p).longValue() == 1L;
    }
    ...
    public static Point liftX(byte[] b) {
        ...
        if(y.modPow(BigInteger.ONE.add(BigInteger.ONE), p).compareTo(y_sq) != 0) {
            return null;
        }
        else {
