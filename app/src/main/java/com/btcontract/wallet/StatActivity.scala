  ...
  def INIT(state: Bundle): Unit = {
    if (WalletApp.isAlive && LNParams.isOperational) {
      ...
      WalletApp.txDataBag.db.txWrap {
        ...
        if (txSummary.isFailure && relaySummary.isFailure && paymentSummary.isFailure && channelTxFeesSummary.isFailure) return
        ...
        for (summary <- txSummary) {
          ...
          addFlowChip(slotTitle.flow, getString(stats_item_received) format WalletApp.denom.directedWithSign(summary.received.toMilliSatoshi, 0L.msat, cardOut, cardIn, cardZero, isPlus = true), R.drawable.border_gray)
          addFlowChip(slotTitle.flow, getString(stats_item_sent) format WalletApp.denom.directedWithSign(0L.msat, summary.sent.toMilliSatoshi, cardOut, cardIn, cardZero, isPlus = false), R.drawable.border_gray)
          addFlowChip(slotTitle.flow, getString(stats_item_fees) format WalletApp.denom.directedWithSign(0L.msat, summary.fees.toMilliSatoshi, cardOut, cardIn, cardZero, isPlus = false), R.drawable.border_gray)
        ...
        for (summary <- paymentSummary) {
          ...
          addFlowChip(slotTitle.flow, getString(stats_item_received) format WalletApp.denom.directedWithSign(summary.received, 0L.msat, cardOut, cardIn, cardZero, isPlus = true), R.drawable.border_gray)
          addFlowChip(slotTitle.flow, getString(stats_item_sent) format WalletApp.denom.directedWithSign(0L.msat, summary.sent, cardOut, cardIn, cardZero, isPlus = false), R.drawable.border_gray)
          addFlowChip(slotTitle.flow, getString(stats_item_fees) format WalletApp.denom.directedWithSign(0L.msat, summary.fees, cardOut, cardIn, cardZero, isPlus = false), R.drawable.border_gray)
        ...
        for (summary <- relaySummary) {
          ...
          addFlowChip(slotTitle.flow, getString(stats_item_earned) format WalletApp.denom.directedWithSign(summary.earned, 0L.msat, cardOut, cardIn, cardZero, isPlus = true), R.drawable.border_gray)
          statContainer.addView(slotTitle.view)
        }
