...
case class CommitmentSpec(feeratePerKw: FeeratePerKw, toLocal: MilliSatoshi, toRemote: MilliSatoshi, htlcs: Set[DirectedHtlc] = Set.empty) {
...
object CommitmentSpec {
  ...
  def fulfillIncomingHtlc(spec: CommitmentSpec, htlcId: Long): CommitmentSpec = spec.findIncomingHtlcById(htlcId) match {
  ...
  def failOutgoingHtlc(spec: CommitmentSpec, htlcId: Long): CommitmentSpec = spec.findOutgoingHtlcById(htlcId) match {
    case Some(htlc) => spec.copy(toLocal = spec.toLocal + htlc.add.amountMsat, htlcs = spec.htlcs - htlc)
    case None => throw new RuntimeException
  }
